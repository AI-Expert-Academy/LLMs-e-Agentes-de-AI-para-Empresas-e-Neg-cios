...
def config_retriever():
  # Carregar documentos
  docs_path = Path("projetos/projeto2/")
...
st.session_state.is_atendimento_already_disabled = True
...
if st.button("Iniciar atendimento"):
  with st.spinner("Aguarde enquanto te transferimos para um atendente..."):
    ...
    st.session_state.is_atendimento_already_disabled = False

placeholder_text = (
    "Aperte o botão 'Iniciar o atendimento' primeiro..." if st.session_state.is_atendimento_already_disabled else "Digite sua mensagem aqui..."
)

input = st.chat_input(placeholder=placeholder_text, disabled=st.session_state.is_atendimento_already_disabled)

if "chat_history" not in st.session_state:
  st.session_state.chat_history = [
      AIMessage(content = "Olá, sou o seu assistente virtual! Para iniciar seu atendimento apertando o botão 'Iniciar o atendimento'."),
  ]
